...
class GorQueryRDD(sparkSession: SparkSession, commandsToExecute: Array[String], commandSignatures: Array[String], cacheFiles: Array[String], projectDirectory: String, cacheDirectory: String, configFile: String, aliasFile: String, jobIds: Array[String], secCtxs: Array[String], allowFail: Array[java.lang.Boolean], redisUri: String, redisKey: String) extends RDD[String](sparkSession.sparkContext, Nil) {
  ...
  override def compute(partition: Partition, context: TaskContext): Iterator[String] = {
    ...
    val (jobId, commandSignature, commandToExecute, cacheFile, securityContext, allow) = (jobIds(i), commandSignatures(i), commandsToExecute(i), cacheFiles(i), if (secCtxs != null) secCtxs(i) else null, if (allowFail != null) allowFail(i) else false)
    ...
    try {
      ...
      relCachePath = if (projectDirectory.nonEmpty && PathUtils.isAbsolutePath(cacheFilePath)) {
      ...
      case e: Exception => {
        if (!allow.asInstanceOf[java.lang.Boolean]) throw e
        val sw = new StringWriter()
        val pw = new PrintWriter(sw)
        e.printStackTrace(pw)
        pw.close()
        val stacktrace = sw.toString.replace("\t",",")
        relCachePath = "[Failed]: "+stacktrace
      }
    }
